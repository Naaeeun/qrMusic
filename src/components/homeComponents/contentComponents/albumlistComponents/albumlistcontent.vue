    ...
    <div class="musiclistcontent">
        ...
        <div class="content">
            <div class="musicContent" v-for="(item,i) in musicList" :key="i" @dblclick="play(i)">
                ...
                <div class="right">
                    <span @click="$router.push(`/singer?id=${item.ar[0].id}`)">{{item.ar[0].name}}</span>
...
export default {
    ...
    computed:{
        ...mapState(['controllerMusic','sumMusicMessage'])
    },
    methods:{
        ...mapMutations(['setSumMusicMessage','setSumMusicUrl','clearMusitList','setControllerMusic','setMusicUrl']),
        async play(i){
            this.clearMusitList()
            let result = await api1.getMusicUrl(this.musicListId)
            let result1 = await api1.getMusicUrl(this.musicListId[i])
